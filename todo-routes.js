...
app.put('/todo/:todoId', function (req, res) {
  ...
  Todo.findByIdAndUpdate(lectionId, { $set: { text: req.body.text } }, { new: true }, function (err, updated) {
    if (err) {
      return res.json({ "success": false, "msg": "Error while deleting Todo", "error": err });
    }

    console.log(updated);
    res.status(200).json({ "success": true, "msg": "Todo updated", "result": updated });
  });
});